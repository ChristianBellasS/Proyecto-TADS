<div class="container-fluid">
    {!! Form::open(['route' => ['admin.vehicles.store-images', $vehicle->id], 'method' => 'POST', 'files' => true, 'id' => 'imagesForm']) !!}

    <!-- Header con información del vehículo MEJORADO -->
    ...
    <!-- Sección de Subir Nuevas Imágenes MEJORADA -->
    <div class="upload-section mb-4">
        <div class="card border-0 shadow-lg">
            ...
            <div class="card-body p-4">
                ...
                <!-- Vista Previa de Nuevas Imágenes MEJORADA -->
    ...
    <!-- Sección de Imágenes Existentes MEJORADA -->
                    ...
                    <div class="existing-images-grid">
                        ...
                        <div class="image-card-existing" data-image-id="{{ $image->id }}">
                            <div class="image-container">
                                ...
                                @else
                                <div class="profile-crown" onclick="setExistingAsProfile({{ $image->id }})" title="Establecer como imagen principal">
                                    <i class="fas fa-crown"></i>
                                </div>
                                @endif
                            </div>
    ...
    <!-- Footer MEJORADO -->
    <div class="modal-footer bg-light mt-4 py-3">
        <button type="button" class="btn btn-outline-secondary btn-lg" data-dismiss="modal">
            <i class="fas fa-times mr-2"></i>Cancelar
        </button>
        <button type="submit" class="btn btn-success btn-gradient btn-lg" id="btnSubmit">
...
$(document).ready(function() {
    ...
    window.setExistingAsProfile = function(imageId) {
        ...
        $('.image-card-existing').each(function() {
            ...
            if (isTarget) {
                card.find('.profile-crown').html('<i class="fas fa-crown"></i><span>Principal</span>').addClass('profile-badge');
                card.find('.profile-badge').show();
            } else {
                card.find('.profile-crown').html('<i class="fas fa-crown"></i>').removeClass('profile-badge');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                if (!imagesToDelete.includes(imageId)) {
                    ...
                    imageCard.find('.delete-btn').addClass('btn-dark').html('<i class="fas fa-check"></i>');
                    
                    $('#imagesToDelete').val(imagesToDelete.join(','));
                    
                    if ($('#profileImageId').val() == imageId) {
                        const remainingImages = $('.image-card-existing').not('.marked-for-deletion');
    ...
    window.setNewAsProfile = function(index) {
        ...
        $('.image-card-existing').each(function() {
            $(this).find('.image-preview').removeClass('profile-active').addClass('profile-inactive');
